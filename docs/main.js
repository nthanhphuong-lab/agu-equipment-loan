...
btnCreateLoan.onclick=async()=>{
  ...
  try{
    ...
    if(eq.quantity_available<qty){ loanCreateMsg.textContent="Số lượng không đủ."; return; }
    ...
    loanCreateMsg.textContent="Tạo yêu cầu thành công.";
...
function renderLoanCard(id,d,adminView=false,currentUserId=null){
  ...
  let userControls="";
  if(!adminView && d.userId===currentUserId && d.status==="pending"){
    userControls+=`<div style="margin-top:6px">
      <button onclick="editMyLoan('${id}')">Chỉnh sửa</button>
      <button onclick="deleteMyLoan('${id}')">Xóa</button>
    </div>`;
  }
...
async function refreshMyLoans(){
  ...
  try{
    ...
    let html="";
    snap.forEach(docSnap=>{ html+=renderLoanCard(docSnap.id, docSnap.data(), false, currentUser.uid); });
...
async function refreshAllLoans(){
  ...
  try{
    ...
    const snap=await getDocs(qRef);
    let html="";
    snap.forEach(docSnap=>{ html+=renderLoanCard(docSnap.id, docSnap.data(), true); });
    allLoans.innerHTML=html||"<p>Chưa có yêu cầu.</p>";
  }catch(e){ console.warn(e); allLoans.innerHTML="<p>Không tải được.</p>";}
}

// ================== EXPORT ==================
// Bạn có thể thêm các hàm approveLoanWithDates, rejectLoan, extendLoan, returnLoanWithTime, editMyLoan, deleteMyLoan ở đây
